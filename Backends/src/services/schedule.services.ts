...
export const addScheduleByDayServices = (day:string, time:string, task: string,data_id:number) :Promise<void> => {
    return new Promise((resolve,rejects) => {
        addScheduleByDay(day,time, task, data_id,(err, results) => {
            if(err) rejects(err);
            resolve(results)
        })
    })
